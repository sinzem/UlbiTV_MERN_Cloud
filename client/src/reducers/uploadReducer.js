...
export default function uploadReducer(state = defaultState, action) {
    switch (action.type) {
            case SHOW_UPLOADER: return {...state, isVisible: true} /* (выведение загрузок на экран) */
            case HIDE_UPLOADER: return {...state, isVisible: false} /* (убрать заагрузки с экрана) */
            case ADD_UPLOAD_FILE: 
                return {...state, files: [...state.files, action.payload]} /* (добавляем загрузку в массив загрузок) */
            case REMOVE_UPLOAD_FILE: 
                return {...state, files: [...state.files.filter(file => file.id !== action.payload)]} /* (удаляем загрузку из массива загрузок) */
